...
#### Registers
The KENBAK-1 has three actively used registers, `a`, `b`, and `x`. The KENBAK-1 also allows you to directly access the RAM addresses that back those registers. With BespokeASM, these two actions are distinct. When the register is a an enumerated operand for the instruction, for example the `ADD` addition operation where the register dictates the first two bits of the instruction byte code, then the register is referred to by the lower case `a`, `b`, or `x`. If instead you are attempting to access the memory address that backs the register via one of the addressing modes operands, then you should use one of the preefined memory labels that maps to the register's address (see below).
