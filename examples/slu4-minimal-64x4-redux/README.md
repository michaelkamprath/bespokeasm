...
### Flow Counters
The ISA configuration declares a `stack` [flow counter](https://github.com/michaelkamprath/bespokeasm/wiki/Instruction-Set-Configuration-File#flow-counters) class that lets **BespokeASM** track MinOS hardware stack depth at assembly time. Code that opts in with `#track stack ...` gets push/pull balance checking on every execution path, and can name stack slots with `.name := COORDINATE(stack, N)` so that `OFFSET(.name)` always resolves to the slot's current `sp`-relative offset — inserting or removing a `phs` no longer requires hand-fixing every `lds`/`sds` offset below it. The metadata is analysis-only: it changes no emitted byte code, and existing programs that do not use the feature assemble exactly as before. Because the stack pointer is memory-mapped at `0xffff`, the class also watches writes to that address: every instruction that writes memory through a compile-time absolute-address operand (the direct stores, clears, increments/decrements, read-modify-write forms, and the immediate/memory move families, in both their absolute and zero-page addressings) declares its write-target operand with `flow_write_operands`, so any such write that resolves to `0xffff` — `spinit`, a direct `mib ...,0xffff`, `sdb 0xffff`, `clb 0xffff`, `miw ...,0xffff`, and so on — makes an active stack counter indeterminate and invalidates its prior coordinates; use `#resume stack = <value>` afterward when deliberately reinitializing the stack inside a region. Two deliberate limits: register-indirect/pointer stores (the `**Z`/`**addr` forms, `szp`, and SP-relative `sds`) are not watched — code that rewrites the SP cell through a pointer must bracket it with `#suspend`/`#resume` itself — and matching is exact on the write's start address, so a multi-word write beginning below `0xffff` (e.g. `miw` to `0xfffe`) is not detected.

See [`software/flow-stack-demo.min64x4r`](software/flow-stack-demo.min64x4r) for a small, heavily commented demonstration: a caller that pushes two arguments under a tracked region, and a `#track stack mode=called` subroutine that names its caller-argument coordinates (accounting for the 2-byte return address `jps` pushes at `sp+1..sp+2`), pushes a local so the argument offsets demonstrably shift, and returns through the `rts` balance check.
