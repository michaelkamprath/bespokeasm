...
class InstructionSet(dict):
    def __init__(self, config_dict: dict, operand_set_collection: OperandSetCollection, default_endian: str):
        self._config = config_dict
        for mnemonic, instr_config in self._config.items():
            mnemonic = mnemonic.lower()
            self[mnemonic] = Instruction(mnemonic, instr_config, operand_set_collection, default_endian)
