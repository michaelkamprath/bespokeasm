...
93. Under `--no-static-analysis` the parameters are ignored like every other analysis-only parameter; instance bounds never appear in generated ISA documentation; the `#track` hover/directive documentation and wiki syntax page document the parameters (collateral definition-of-done).
...
98. A non-list/duplicate/out-of-range `invalidate_on_write` address, or a non-list/duplicate/out-of-range `flow_write_operands` source index, → config-load error with analysis enabled; disabled analysis leaves the metadata uninterpreted.
99. An instruction or selected variant with `flow_invalidates: [stack]` unconditionally makes every active `stack` instance indeterminate, invalidates its coordinates, and requires `#resume` before precise use. The behavior composes through macros. A non-list value, duplicate class, or undeclared class → config-load error with analysis enabled and is uninterpreted when analysis is disabled.
