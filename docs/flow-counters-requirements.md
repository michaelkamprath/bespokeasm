...
* flow directives, flow-dependent `#assert` directives, and well-formed `:=` declarations are recognized as analysis-only syntax but otherwise ignored: they emit nothing, open no regions, create no numeric values, enter no normal symbols, and produce no flow diagnostics. A general `#assert` that contains no flow dependency remains active and is evaluated normally. For ordinary compilation semantics, ignoring the flow-only lines is equivalent to stripping them;
* malformed flow syntax remains a source error even when analysis is disabled. A syntactically well-formed parameter belonging to a future milestone may be ignored, but a missing counter name, malformed `name=value` parameter, invalid expression, or invalid counter name is never silently accepted;
...
Thus a source file may retain optional flow assertions and tracking annotations when analysis is disabled, while general assertions continue to enforce ordinary preprocessing invariants. It cannot retain a dependency on an analysis-computed emitted value without supplying an ordinary literal/constant alternative.
...
* Flow-specific words join the assembler's reserved namespace only when the selected ISA has a `flow_counters` section. A non-flow ISA may continue to use `track`, `endtrack`, `set`, `suspend`, `resume`, `COUNTER`, `COORDINATE`, or `OFFSET` as ordinary symbols or configured mnemonics. `assert` is a base preprocessor keyword in every ISA because general `#assert` does not require flow analysis. Generated editor extensions follow the same capability gate.
* Using any flow-counter construct (`#track` and the other flow directives, a flow-dependent `#assert`, `:= COORDINATE()`, `COUNTER()`, or `OFFSET()`) in source compiled against an ISA that does not enable the feature is an **error** — "this instruction set does not enable flow counters" — and that error fires **only because the construct was used**. A general `#assert` remains valid without flow counters.
...
* Flow-control directives (`#set`, `#suspend`, `#resume`, `#endtrack`, `#entry`), the flow shorthand form of `#assert`, the first argument of `COORDINATE()`, and `COUNTER()` reference counter *instances* by name. `OFFSET()` instead references a counter-coordinate symbol, which records the instance from which it was derived.
...
* **Allowed:** `COUNTER()` and `OFFSET()` in fixed-width instruction operand values, fixed-size data element values, and flow-aware directives such as `#assert`, `#set`, and `#resume`; `COORDINATE()` only as the complete right-hand side of a `:=` counter-coordinate declaration.
...
* `#if` / `#elif` retain their existing compilation-selection role and may use only their existing preprocessor inputs. They must never accept `COUNTER()`, `OFFSET()`, counter coordinates, or any other static-analysis result: analysis runs only after the source to compile has been selected, and an analysis result must not control source inclusion, instruction selection, layout, or emitted word count.
...
* In a flow-enabled ISA, either side may also contain `COUNTER()` or `OFFSET()`. `#assert <counter> <comparison> <expr>` remains shorthand for `#assert COUNTER(<counter>) <comparison> <expr>` when the otherwise-undefined left name denotes a scalar flow-counter instance. An explicit `COUNTER(name)` is preferred where the shorthand would be ambiguous with a preprocessor macro. A flow assertion is purely a checkpoint: assembly errors when the comparison is false and otherwise forwards the incoming state unchanged. It does **not** collapse a join, discard an infeasible path, or re-anchor a value.
* `#set <counter> = <expr>` — re-anchors a **scalar** tracked value when the programmer knows better than the assembler (e.g., after a stack-pointer manipulation the effect model cannot express). This is a programmer assertion and is taken on faith. In the CFG it transforms each incoming scalar state into the asserted scalar outgoing state; paths that bypass it remain unchanged and reconcile normally at a later join. The asserted value is still checked against the class's `min_value`/`max_value` bounds — a programmer asserting a value the class declares impossible is most likely a mistake — and, per the dead-coordinate rule, `#set` permanently invalidates the counter's prior coordinates. `#set`, `#suspend`, and `#resume` are scalar-only through the committed M0–M5 scope; applying them to an `interval` counter is an explicit unsupported-operation error. Their interval behavior, if any, is deferred to the provisional M6 design rather than implicitly collapsing a hull.
...
36. `#suspend` silences tracking; `COUNTER()` / `OFFSET()` reference, new `:=` coordinate declaration, or `flow_terminal` instruction while suspended → errors.
...
69. At **each milestone**, extensions generated from a flow-enabled ISA include every **then-shipped** flow directive, declaration, and expression form in all three grammars (VS Code, Sublime, Vim), with hover docs present; extensions generated from a non-enabled ISA contain none of the flow-counter tokens. General `#assert` highlighting and hover documentation appear for every ISA, while its flow operands use flow-specific captures only in enabled grammars. Every shipped directive/operator/declaration has a `directive_docs.py` entry (enforced by the existing docsgen test). The flow fixture matrix grows as syntax ships: M1 (`#track`, `#endtrack`, `COUNTER()`); M2 (`:=`, `COORDINATE()`, `OFFSET()`); M4 (flow-aware `#assert`, `#set`, `#suspend`, `#resume`); M5 (`#entry`); M6 (`COUNTER(x).min` / `.max` assertion forms); M7 (`#loop`).
...
76. Under `--no-static-analysis`, a source containing only flow directives, flow-dependent assertions, and unused `:=` declarations assembles byte-identically to a copy with those constructs stripped and emits no flow diagnostics. General assertions still execute. Analysis-only syntax also succeeds against an ISA with no `flow_counters` section.
...
**Collateral definition-of-done (every visible milestone from M1 on): new language surface ships with its collateral.** M1 builds the pipeline with its first directives/operators and the static-analysis CLI switch; afterward, any milestone that adds or activates source syntax, an expression form, or a config key must ride it in the same release: keyword-registry entries (`keywords.py`), hover/doc entries (`directive_docs.py`), regenerated-extension verification, wiki section, and CLI documentation where applicable. Concretely: `:=`, `COORDINATE()`, and `OFFSET()` land with **M2**; `#assert`/`#set`/`#suspend`/`#resume` with **M4**; `#entry` with **M5**; interval selectors / assertion forms with **M6**; and `#loop` with **M7**. Config-key documentation (`coordinate_offsets`, `allow_zero_offset`, `join`, `entry_modes`, call summaries, edge maps) lands with the milestone that activates each. The changelog records major user-usable flow-counter functionality as it becomes available, without becoming a milestone-by-milestone implementation ledger. A milestone is not done while its syntax is invisible to the editors.
