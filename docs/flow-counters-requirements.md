...
* verify programs by default; `--no-flow-checks` suppresses optional flow
  ...
  `OFFSET()` values still trigger the minimum analysis needed to resolve them.
...
* if an instruction operand or fixed-size data value contains `COUNTER()` or
  `OFFSET()`, the graph pass runs automatically and pre-resolves those values.
...
`OFFSET()`, coordinates, and all other analysis results remain forbidden in
`#if` / `#elif`.
...
77. Under `--no-flow-checks`, an instruction operand or fixed-size data value containing `COUNTER()` or `OFFSET()` automatically triggers flow propagation and resolves to the same emitted value as the default checked compilation. A requested value that is structurally ambiguous, indeterminate, inactive, or otherwise unsafe still errors; no numeric fallback is emitted.
78. Under `--no-flow-checks`, a coordinate declaration required by an emitted `OFFSET()` is created and resolved normally. Unused declarations remain zero-width and do not force graph propagation. Direct use of a coordinate name without `OFFSET()` remains invalid.
...
* **Counter-coordinate symbol records.** Extend the scoped-symbol machinery with a distinct `COUNTER_COORDINATE` kind carrying the resolved integer, owning counter instance, declaration site, and validity state. The dependency-driven pass inserts the coordinate when an emitted `OFFSET()` requires it; an unused declaration remains zero-width and need not trigger propagation.
...
**CLI documentation:** `Installation-and-Usage.md`, `bespokeasm compile --help`, and shell-completion fixtures document `-a`/`--flow-checks` and `-A`/`--no-flow-checks`, the enabled default, suppression of optional verification/listing annotations, and automatic resolution of emitted `COUNTER()` / `OFFSET()` dependencies.
