...
* Flow-specific words join the assembler's reserved namespace only when the selected ISA has a `flow_counters` section. A non-flow ISA may continue to use `track`, `endtrack`, `COUNTER`, `COORDINATE`, or `OFFSET` as ordinary symbols or configured mnemonics. Generated editor extensions follow the same capability gate.
